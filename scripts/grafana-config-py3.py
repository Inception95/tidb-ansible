...
def export_dashboard(api_url, api_key, dashboard_name):
    req = urllib.request.Request(api_url + 'api/dashboards/db/' + dashboard_name,
...
def import_dashboard_via_user_pass(api_url, user, password, dashboard):
    payload = {'dashboard': dashboard,
               'overwrite': True}
    #auth_string = base64.b64encode(('%s:%s' % (user, password)).encode("utf-8"))
    info_string = user + ":" + password
    info_bytes = info_string.encode("ascii")
    info_bytes_base64 = base64.b64encode(info_bytes)
    auth_string = info_bytes_base64.decode("ascii")
...
if __name__ == '__main__':
    for type_ in src['dashboards']:
        ...
        for dest in dests:
            ...
            print(dest['url'], dest['user'], dest['password'])
